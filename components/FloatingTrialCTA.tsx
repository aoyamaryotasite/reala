...
export default function FloatingTrialCTA({
  ...
  zIndex = 60,
  showOnMobile = true,
  revealOffset = 500, // ← 何pxスクロールで表示するか
}: {
  href?: string;
  label?: string;
  zIndex?: number;
    ...
    const onScroll = () => {
      if (!ticking) {
        window.requestAnimationFrame(() => {
          setRevealed(window.scrollY >= revealOffset);
          ticking = false;
        });
        ticking = true;
      }
  ...
  const isIn = mounted && revealed;

  return (
    <>
      <div
        className={`cta-root ${isIn ? "in" : ""}`}
        style={{ zIndex }}
        aria-hidden={!isIn}
      >
      ...
      <style jsx>{`
        ...
        /* モバイル表示可否（吹き出しだけ制御。FABはスクロール後に出現） */
        ...
        .cta-link:hover .cta-fab {
          ...
          box-shadow: 0 16px 32px rgba(0,0,0,.16), 0 6px 12px rgba(0,0,0,.1);
        }
